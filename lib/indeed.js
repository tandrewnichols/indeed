...
Indeed.prototype = {
  test: function(currentOnly) {
    ...
    if (this.current.length) {
      val = !!(_[last.join](this._compare(this.current, this.current.pop()), (last.val ^ last.negate)) ^ this.groupNegate);
    } else {
      val = !!((last.val ^ last.negate) ^ this.groupNegate);
    }
    if (this.previous.length && !currentOnly) {
      var method = _(this.previous).last().join;
      var result = this._compare(this.previous, this.previous.pop());
  ...
  _compare: function(list, item) {
    if (list.length) {
      ...
      var result = this._compare(list, list.pop());
  ...
  is: function(condition) { 
    ...
    if (!current.compared) {
      current.val = current.actual === condition;
      current.compared = 'is';
    } else {
  ...
  equals: function(condition) {
    var current = this.current.pop();
    if (!current.compared) {
      current.val = _(current.actual).isEqual(condition);
      current.compared = 'equals';
    } else {
