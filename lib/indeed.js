...
_.mixin({
  compare: function(list, previous) {
    if (list.length) {
      return _[previous.join](_(list).compare(list.shift()), previous.val);
    } else {
      return previous.val;
    }
  },
...
Indeed.prototype = {
  test: function() {
    ...
    if (this.previous.length) {
      var previous = this.previous.shift();
      return _[previous.join](_(this.previous).compare(previous), val);
    } else {
      return val; 
    }
  },
  ...
  get And() {
    ...
    this.allowed = ['indeed', 'also', 'neither', 'either'];
    return this;
  },
